...
def mmotifs(
    ...
    P = P.copy()
    if excl_zone_denom is not None:
    ...
    while len(motif_distances) < max_motifs:
        mdls, subspaces = mdl(T, m, candidate_idx, nn_idx, include,excl_zone_denom=excl_zone_denom)
        if k is None:
            k = np.argmin(mdls)
            reset_k = True
        ...
        for idx in query_matches[:, 1]:
            apply_exclusion_zone(P, idx, excl_zone, np.inf)
        candidate_idx = np.argmin(P, axis=1)
        nn_idx = I[np.arange(len(candidate_idx)), candidate_idx]
        if reset_k:
            k = None

    motif_distances = _jagged_list_to_array(
        motif_distances, fill_value=np.nan, dtype=np.float64
    )
    motif_indices = _jagged_list_to_array(
        motif_indices, fill_value=-1, dtype=np.int64
    )

    return motif_distances, motif_indices, motif_subspaces, motif_mdls

#adaptation of stumpy.mdl for known subspaces
def mdl_known_subspaces(
    T,
    m,
    subseq_idx,
    nn_idx,
    ...
    """
    T = _preprocess(T)
    check_window_size(m, max_size=T.shape[1], n=T.shape[1])
    ...
    for k in range(len(subspaces)):
        subseqs, _, _, _ = preprocess(T[:, subseq_idx[k] : subseq_idx[k] + m], m)
        subseqs = z_norm(subseqs, axis=1)
        subseqs[T_subseq_isconstant[:, subseq_idx[k]]] = 0.0

        neighbors, _, _, _ = preprocess(T[:, nn_idx[k] : nn_idx[k] + m], m)
        neighbors = z_norm(neighbors, axis=1)
        neighbors[T_subseq_isconstant[:, nn_idx[k]]] = 0.0

        disc_subseqs = discretize_func(subseqs)
        disc_neighbors = discretize_func(neighbors)

        bit_sizes.append(_mdl(disc_subseqs, disc_neighbors, subspaces[k], n_bit=n_bit))

    return np.array(bit_sizes)
...
def mdl(
    ...
    """
    T = _preprocess(T)
    check_window_size(m, max_size=T.shape[1], n=T.shape[1],excl_zone_denom=excl_zone_denom)
    T_subseq_isconstant = process_isconstant(T, m, T_subseq_isconstant)
...
def match(
    ...
    Q_subseq_isconstant = process_isconstant(Q, m, Q_subseq_isconstant)
    T_subseq_isconstant = process_isconstant(T, m, T_subseq_isconstant)
...
def get_max_window_size(n, excl_zone_denom=None):
    """
    ...
    """
    if excl_zone_denom is None:
        excl_zone_denom = config.STUMPY_EXCL_ZONE_DENOM
    max_m = (
        int(
            n
            - np.floor(
...
def check_window_size(m, max_size=None, n=None, excl_zone_denom=None):
    """
    ...
    if n is not None:
        ...
        if excl_zone_denom is not None:
            excl_zone = int(math.ceil(m / excl_zone_denom))
        else:   
